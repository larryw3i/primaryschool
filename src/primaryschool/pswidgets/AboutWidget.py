...
class AboutWidget(WidgetABC):
    def __init__(
        self, top_widget=None, app_name_fontsie=20, app_version_fontsize=10
    ):
        super().__init__()
        ...
        self.app_name_label = tk.Label(
            self.toplevel, text=app_name, font=("", self.app_name_fontsize)
        )
    ...
    def get_app_dep_labels(self):
        if not self.app_dep_labels:
            for name, version, home_url, licelse, license_url in app_deps:
                ...
                dep_label.bind(
                    "<Button>",
                    lambda hu=home_url: self.cmd_app_dep_label_b(dep_url=hu),
                )
