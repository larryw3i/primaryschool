import os
import sys
from pathlib import *

from primaryschool.psdep import *

project_name = app_name = "primaryschool"
project_vesion = app_version = "0.0.24"
project_author = app_author = "larryw3i"
project_author_email = app_author_email = "larryw3i@163.com"
project_maintainer = app_maintainer = app_author
project_maintainer_email = app_maintainer_email = app_author_email
project_description = app_description = "primary school knowledge games"
project_url = (
    project_source_url
) = app_url = "https://github.com/larryw3i/primaryschool"
project_authors = app_authors = project_contributors = app_contributors = [
    {"name": app_author, "email": app_author_email},
]
project_reqs = app_reqs = setup_reqs
project_license = app_license = (
    "MIT",
    "https://github.com/larryw3i/primaryschool/blob/main/LICENSE",
)

project_sponsors = app_sponsors = []


pass
